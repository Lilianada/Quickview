      ...
      <div id="experience" className="lg:min-h-screen">
        ...
        {experiences.map((exp, index) => (
          <div key={index} className="mb-10 lg:flex lg:mb-6 lg:p-4">
            <h3 className={`font-semibold text-lg lg:hidden`}>{exp.company}</h3>
            <p className={`mt-1 text-xs leading-4 lg:w-1/4 lg:mt-[6px]`}>
              {exp.period}
            </p>
            <p className="mt-4 leading-4 text-xs lg:hidden opacity-50 tracking-widest uppercase">
              {exp.role}
            </p>
            {/* desktop */}
            ...
            <p className="lg:hidden text-sm leading-5 opacity-80 mt-1 lg:mt-3">
              {exp.description}
            </p>
          </div>
        ))}
